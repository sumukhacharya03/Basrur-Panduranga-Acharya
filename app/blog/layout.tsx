...
// This component uses browser-only APIs like `window` and `navigator`.
function CopyButton() {
  const [text, setText] = useState('Copy')
  const [currentUrl, setCurrentUrl] = useState('')

  // Set the URL only after the component has mounted on the client.
  useEffect(() => {
...
export default function LayoutBlogPost({
  ...
  // This state ensures that components needing the `window` object
  ...
  return (
    <>
      ...
      <div className="absolute right-4 top-24">
        {/* The CopyButton will only be rendered if isClient is true. */}
        {isClient && <CopyButton />}
      </div>
